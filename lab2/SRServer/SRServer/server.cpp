...
const int SEND_WIND_SIZE = 10;//发送窗口大小为 10， SR 中应满足 Ws + Wr <= N（Ws 为发送窗口大小, Wr为接收窗口， N 为序列号个数）
							  //本例取序列号 0...19 共 20 个
							  //如果将窗口大小设为 1，则为停-等协议
const int SEQ_SIZE = 20; //序列号的个数，从 0-19 共计 20 个
						 //由于发送数据第一个字节，如果值为 0，则数据会发送失败
						 //因此接收端序列号为 1-20，与发送端一一对应
int ack[SEQ_SIZE];  //收到 ack 情况，对应 0-19 的 ack, -1:已发送未确认，0:未发送，1：已发送已确认
...
//************************************
void timeoutHandler() {
	printf("Timer out error. curAck: %d\n", curAck);
	ack[curAck] = 0;   //超时后设置为未发送
	totalSeq -= 1;
	curSeq = curAck;
}
...
void ackHandler(char c) {
	...
	if (index == curAck){
		if (index != 0) {
			int tmpCurAck = curAck;
			while (ack[tmpCurAck] == 1) {
				tmpCurAck = (tmpCurAck + 1) % SEQ_SIZE;    //移动窗口直到发现未确认分组
				printf("window moving, curAck: %d\n", tmpCurAck);
				if (tmpCurAck == curAck) {  //循环了一个轮回，所有的序列号ack都为TRUE				
...
int main(int argc, char* argv[])
{
	...
	while (true) {
		...
		if (strcmp(buffer, "-time") == 0) {
			...
			ZeroMemory(buffer, sizeof(buffer));
			int recvSize;
			int waitCount[SEQ_SIZE] = { 0 };   // 对每一个Seq维持一个定时器
			int connectCount = 0;
