...
//收到 ack 情况，对应 0-19 的 ack
//FALSE代表seq已发但未确认,TRUE代表已发确认或未发
BOOL ack[SEQ_SIZE];  
...
void ackHandler(char c) {
	...
	if (curAck <= index) { 
		//比如已发未确认的seq为13,14,15,16,17，当收到ack=15(index=14)则
		//代表13,14都为已发确认, curAck为15
		//注意对于发送端而言seq在0-19之间，对于接收端而言是1-20
		...
		//比如已发未确认的seq为18,19,1,2，当收到ack=2(index=1)则
		//代表18,19,1都为已发确认，故curAck = index+1，即为2
